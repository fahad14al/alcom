...
class PageViewModelTests(TestCase):
    ...
    def test_create_page_view(self):
        ...
        self.assertEqual(str(page_view), 'pagetest - /products/')
...
class ProductViewModelTests(TestCase):
    ...
    def test_create_product_view(self):
        ...
        self.assertEqual(str(product_view), 'productviewtest - Test Product')
...
class AnalyticsManagerTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # Create page views
        PageView.objects.create(page_url='/', session_id='1', created_at=timezone.make_aware(datetime.combine(today, datetime.min.time())))
        PageView.objects.create(page_url='/products/', session_id='2', created_at=timezone.make_aware(datetime.combine(yesterday, datetime.min.time())))
        
        # Create product views
        ProductView.objects.create(product=cls.product, session_id='3', created_at=timezone.make_aware(datetime.combine(today, datetime.min.time())))
        ProductView.objects.create(product=cls.product, session_id='4', created_at=timezone.make_aware(datetime.combine(last_week, datetime.min.time())))
...
class SalesReportAPITests(APITestCase):
    ...
    def test_generate_daily_report(self):
        ...
        OrderItem.objects.create(order=order1, product=product, quantity=2, price=Decimal('100.00'))
        OrderItem.objects.create(order=order2, product=product, quantity=1, price=Decimal('150.00'))
