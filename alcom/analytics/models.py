...
class ProductViewManager(AnalyticsManager):
    def get_popular_products(self, limit=5):
        # Return Products authenticated with view count, ordered by count
        # We need to import Product inside to avoid circular import if possible, 
        # but Product is imported at top.
        return Product.objects.annotate(view_count=Count('views')).order_by('-view_count')[:limit]
