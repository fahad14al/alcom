...
class RatingAndReviewModelTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='ratingtest', email='ratingtest@example.com', password='pass')
...
class ReviewAPITests(APITestCase):
    ...
    def test_create_review(self):
        ...
        url = f'/api/reviews/products/{self.product.slug}/reviews/'
        data = {
            'rating': 5,
            'title': 'Excellent!',
    ...
    def test_list_reviews_for_product(self):
        ...
        Review.objects.create(rating=rating, title='Good', comment='It was okay.', is_approved=True)
        ...
        if 'results' in response.data:
            ...
            self.assertEqual(response.data['results'][0]['title'], 'Good')
        else:
    ...
    def test_get_review_detail(self):
        ...
        rating = Rating.objects.create(product=self.product, user=self.user, rating=5)
        review = Review.objects.create(rating=rating, title='Awesome', comment='Really great.', is_approved=True)
